/**
@author xihale
@brief Main source for the kgm-unlock project
@origin https://github.com/nullptr-0/KgmWasm/blob/master/kgm.hpp
@license Apache-2.0
*/

#include <kgm.hpp>

namespace xihale {
namespace unlock_music {
namespace kgm {
namespace utils {

static constexpr uint8_t VprHeader[] = {0x05, 0x28, 0xBC, 0x96, 0xE9, 0xE4,
                                        0x5A, 0x43, 0x91, 0xAA, 0xBD, 0xD0,
                                        0x7A, 0xF5, 0x36, 0x31};
static constexpr uint8_t KgmHeader[] = {0x7C, 0xD5, 0x32, 0xEB, 0x86, 0x02,
                                        0x7F, 0x4B, 0xA8, 0xAF, 0xA6, 0x8E,
                                        0x0F, 0xFF, 0x99, 0x14};
static constexpr uint8_t VprMaskDiff[] = {0x25, 0xDF, 0xE8, 0xA6, 0x75, 0x1E,
                                          0x75, 0x0E, 0x2F, 0x80, 0xF3, 0x2D,
                                          0xB8, 0xB6, 0xE3, 0x11, 0x00};

static constexpr uint8_t Table1[] = {
    };

static constexpr uint8_t Table2[] = {
    };

static constexpr uint8_t MaskV2PreDef[] = {
    ,
};

uint8_t get_mask(std::size_t pos) {
  std::size_t offset = pos >> 4;
  uint8_t value = 0;
  while (offset >= 0x11) {
    value ^= Table1[offset % 272];
    offset >>= 4;
    value ^= Table2[offset % 272];
    offset >>= 4;
  }

  return MaskV2PreDef[pos % 272] ^ value;
}

} // namespace utils

std::vector<char> decrypt(std::span<uint8_t> data, bool is_vpr) {

  std::array<uint8_t, 0x10 + 1> key;
  uint32_t headerLen = *(uint32_t *)(data.data() + 0x10);
  std::copy_n(data.data() + 0x1C, 0x10, key.data());
  key[16] = 0;
  data = data.subspan(headerLen);

  std::vector<char> result(data.size());

  for (std::size_t i = 0; i < data.size(); ++i) {
    uint8_t med8 = key[i % 17] ^ data[i];
    med8 ^= (med8 & 0xf) << 4;

    uint8_t msk8 = utils::get_mask(i);
    msk8 ^= (msk8 & 0xf) << 4;
    result[i] = med8 ^ msk8;

    if (is_vpr) {
      result[i] ^= utils::VprMaskDiff[i % 17];
    }
  }

  return result;
}

} // namespace kgm
} // namespace unlock_music
} // namespace xihale
